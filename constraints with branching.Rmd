...
expand.grid(learningStrategy = c(2),
            numSteps = c(2),
            blockedLayer = c(1),
            propBlocked = c(0.1),
            repl = 1:params$replicateSimulations) %>%
...
node_colors <- rep("forestgreen", gorder(tree))
node_colors[aux_nodes] <- "lightblue"
node_colors[blocked_nodes] <- "firebrick"
...
animation::saveGIF({
  for(t in seq(1,2000,5)) {
    ...
    plot(tree, layout = layout.reingold.tilford(tree), vertex.size=scaled_sizes, vertex.label=NA, vertex.color=node_colors,main = "Blocked Layer = 1", sub = paste("Time Step", t))
    
  }
}, movie.name = "layer_1.gif", interval = 1/50)
```
