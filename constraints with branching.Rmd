...
params <- list(
  ...
  payoff_weight = 1,              # How much the payoff depends on randomness vs distance from the root node (0 for random, 1 for fully deterministic))
  falloffFunction = "reciprocal", # "reciprocal" or "adjacent"
  probDelta = 2,                  # parameter for the falloff function
...
for (learningStrategy in c(0,1,2,3,4)){
  ...
  for (scaling in c(-1, -0.5, 0, 0.5, 1)){
    params$payoff_scaling <- scaling
      ...
      tree <- generate_converging_tree(params, branching_factor)
...
plotPayoffScaling <- function(strategySuccess){
  ...
  stratNames <- c('payoff', 'similarity', 'age', 'conformity')
  ...
  for (ps in sort(unique(strategySuccess$payoff_scaling))[1:length(unique(strategySuccess$payoff_scaling))]) {
    ...
    boxplot(meanPayoff ~ learningStrat, data=b, main=ps, col=stratCols, ylim=c(0,1.5), xaxt='n',xlab="")
