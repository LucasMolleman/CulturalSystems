...
params <- list(
  num_nodes = 14,                 # size of the cultural systems (number of nodes)
  ...
  olderPref = 0,
  payoff_scaling = 5,             # Constant that is added/subtracted for each step away from the root node
  payoff_weight = 0.8,            # How much payoff depends on randomness vs distance from the root node (0=random, 1=deterministic))
  falloffFunction = "reciprocal", # "reciprocal" or "adjacent"
  probDelta = 2.5,                # parameter for the falloff function
  root_node = 14,                 # root node of the tree, should be equal to num_nodes when using converging tree, else 1
  branching_factor = 3,           # branching factor of the tree
  blockedLayer = 1,               # layer at which tree blockages occur
  numBlocked = 4,                 # number of blocked traits in the model
...
prerequisites <- probBonusRequirements(tree)

for (learningStrategy in c(0,1,2,3,4)){
  for (numBlocked in c(3,6,9)){
    params$numBlocked <- numBlocked
    ...
    initResults <- matrix(numeric(0), ncol = ncol(strategySuccess)) 
      ...
      strategySuccess <- runsimulation(params, learningStrategy, repl, tree, prerequisites)
  ...
  for (blockedLayer in c(1,2,3)){
    params$blockedLayer <- blockedLayer
      ...
      for(repl in 1:params$replicateSimulations){
        print(paste("strat=", learningStrategy, "bf=", params$branching_factor, "scaling=",params$payoff_scaling, "repl=", repl))
        sumThisSimulation <- runsimulation(params, learningStrategy, repl, tree, prerequisites)
...
```{r}
plotPayoffScaling(strategySuccess, "numBlocked",'meanPayoffUnblocked')
plotPayoffScaling(strategySuccess, "numBlocked",'meanPayoffBlocked')
```
...
plotPayoffScaling <- function(strategySuccess, independent, dependent){
  strategySuccess <- data.frame(strategySuccess)
  names(strategySuccess) <- c('numNodes', 'branching_factor', 'tree_layers', 'alpha', 'beta', 'learningStrat', 'olderPref', 'repl', 'payoff_scaling', 'blockedLayer', 'numBlocked',  'meanPayoff', 'meanPayoffUnblocked', 'meanPayoffBlocked')
  ...
  for (level in sort(unique(strategySuccess[,independent]))[1:length(unique(strategySuccess[,independent]))]) {
    ...
    boxplot(b[[dependent]] ~ b$learningStrat, main=paste(independent,"=",level), col=stratCols, ylim=c(0,5), xaxt='n',xlab="", ylab=dependent)
