...
params <- list(
  num_nodes = 48,                 # size of the cultural systems (number of nodes)
  ...
  olderPref = 0,
  payoff_scaling = 3,             # Constant that is added/subtracted for each step away from the root node
  ...
  root_node = 48,                 # root node of the tree, should be equal to num_nodes when using converging tree, else 1
...
```{r}
plotPayoffScaling(strategySuccess, "blockedLayer",'meanPayoffUnblocked')
plotPayoffScaling(strategySuccess, "blockedLayer",'meanPayoffBlocked')
```
