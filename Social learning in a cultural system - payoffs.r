...
for (learningStrategy in 3){#c(0,1,2,3,4)){
...
dev.off()
boxplot(meanPayoff ~ olderPref, data=strategySuccess, col=stratCols[4])

meansNocut<-c()
for (i in unique(strategySuccess$olderPref)){
	b<-subset(strategySuccess, olderPref==i)
	meansNocut<-c(meansNocut, mean(b$meanPayoff))
}
par(las=1, lend=1)
plot(-10:20, means0cut, col=stratCols[4], pch=16, cex=1.5, ylim=c(0.2,0.4), 
	main='age-based learning', xlab='preferred age difference', ylab='mean payoff')
lines(-10:20, means0cut, col=stratCols[4])
points(-10:20, meansNocut, col='brown', pch=17, cex=1.5)
lines(-10:20, meansNocut, col='brown')

arrows(0,-1,0,1, code=0)
legend('topleft', c('ignore younger', 'not ignore younger'), bg='white',
	pch=16:17, col=c(stratCols[4], 'brown'))
...
plot(1:max(xx), 1:max(xx)*0+1, type='n', ylim=c(0,1.8), xlim=c(1,max(xx)),
...
for (learnStrat in 0:4){
	...
	summM<-c()
	for (bf in unique(b$branching_factor)){
#	for (nNodes in xx){
		d<-subset(b, branching_factor==bf)
...
for (alph in c(1,3)){
	a<-subset(strategySuccess, alpha==alph)
	for (bet in c(1,3)){
		b<-subset(a, beta==bet)
		
		if (!(alph==3 && bet==3)) boxplot( meanPayoff ~ learningStrat, data=b, col=stratCols, ylim=c(0,0.5))
		
	}
}
legend('topright', c('random', 'payoff', 'similarity', 'age', 'conformity'), col=stratCols, pch=15)
