...
# create lists
ID <- rep(population, (timesteps+1)*rounds)
n_timestep <- rep(rep(0:timesteps, each = length(population)), rounds)
n_rounds <- rep(1:rounds, each = length(population)*(timesteps+1))
n_skills <- c()
meta_strat <- c()
learning_strat <- c()
payoffs <- c()
...
for (r in 1: rounds) {
  ...
  for (n in 1:length(population)){
    r_skills[n] <- sample(seq(1:20), 1, prob = skill_probs)
    r_ages[n] <- r_skills[n]
  ...
  r_meta_strat <- rep(r_meta_strat, timesteps+1)
  ...
  for (t in 1:timesteps) {
    ...
    if (t_skills[individual] == 20){
        new_learner <- TRUE
        while(new_learner == TRUE){
          individual <- sample(population, 1)
          if (t_skills[individual] < 20) new_learner <- FALSE
      }
      }
    # select teachers 
    ...
    if(state == "IL"){ # individual learning 
      ...
      if(t_skills[individual] == 20) t_skills[individual] <- practice_skills()
      if (t_skills[individual] < 20){
    ...
    if(state == "SL"){
      ...
      if(r_meta_strat[individual] == 1){ # fixed meta-strategy
        ...
        if(t_skill_attempt[individual] == (t_skills[individual]+1)){
          t_learner_payoffs[individual] <- t_learner_payoffs[individual] + r_payoffs[t_skills[individual] + 1]
    ...
    r_skills <- append(r_skills, t_skills)
    r_ages <- append(r_ages, t_ages)
    r_learning_strat <- append(r_learning_strat, t_learning_strat)
    r_strat1_success <- append(r_strat1_success, t_strat1_success)
    r_strat2_success <- append(r_strat2_success, t_strat2_success)
    r_strat3_success <- append(r_strat3_success, t_strat3_success)
    r_strat4_success <- append(r_strat4_success, t_strat4_success)
    r_strat1_failure <- append(r_strat1_failure, t_strat1_failure)
    r_strat2_failure <- append(r_strat2_failure, t_strat2_failure)
    r_strat3_failure <- append(r_strat3_failure, t_strat3_failure)
    r_strat4_failure <- append(r_strat4_failure, t_strat4_failure)
    r_teacher_ID <- append(r_teacher_ID, t_teacherID)
    r_skill_attempted <- append(r_skill_attempted, t_skill_attempt)
    r_SL_rate <- append(r_SL_rate, t_SL_rate)
    r_SL_success <- append(r_SL_success, t_SL_success)
    r_IL_rate <- append(r_IL_rate, t_IL_rate)
    r_IL_success <- append(r_IL_success, t_IL_success)
    r_n_resets <- append(r_n_resets, t_resets)
    r_learner_payoffs <- append(r_learner_payoffs, t_learner_payoffs)
    r_eligible_teachers <- append(r_eligible_teachers, t_eligible_teachers)
...
num_columns <- ceiling(length(payoffs) / 20)
payoff_matrix <- matrix(payoffs, ncol = num_columns, byrow = TRUE)
